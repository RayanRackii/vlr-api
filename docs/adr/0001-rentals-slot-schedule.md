...
**Addendum 2026-08-22:** `ScheduleTemplate` rows may overlap across OccupancyKinds on the same Rentable and weekday. Exact duplicates (same tenant, rentable, weekday, start, end, kind) are rejected; the same kind cannot overlap itself. Unpublished SlotGrid days split templates at interval boundaries and let the higher-precedence kind occupy the overlap (Closed > Lesson > Open; equal rank uses the kind key). `PublishDay` still gap-fills by rentable + start and does not rewrite overlapping persisted Slot rows. Weekly apply looks up the full interval+kind tuple, never start time alone.
