...
- [x] Wave 5 follow-up #1 (test-only): convention test `ModuleGateConventionTests` — a new commercial-module controller/action without matching `[RequireActiveModule]` fails in CI (namespace ownership + commercial `RequirePermission`). Isolated regressions: PMOC enable-on-generic update 400; trial `TrialFamilyKeys` must satisfy `HasPmocProvisioningFamily` when trial includes PMOC.
- [ ] Wave 5 follow-up: PermissionResolver `tenantId` vs ambient (deferred; not this slice).
- [ ] Wave 5 follow-up: `UserDirectoryService` module lookup consolidation (deferred; not this slice).
...
| Data | Mudança |
|---|---|
| 2026-09-21 | **Em curso (API, Phase 3 Slice 2):** Coverage V3 `GET /api/maintenance-plans/{id}/coverage` — history/due/attention, `effectiveNextDueDate`, generator visibility. Shared DEV freeze on Phase 2: do **not** apply Phase 3 migration; do **not** deploy develop to Railway DEV. H6/H7 = NO. Spec `docs/plans/active/2026-09-19-pmoc-os-phase3.md`. Branch `feat/pmoc-os-phase3-coverage-v3`. |
...
| 2026-09-19 | **Executado (API, Phase 2 Slice 1 + PROD):** derived PMOC coverage read model `GET /api/maintenance-plans/{id}/coverage` (calendar due model, superseded by Phase 3). H6/H7 = NO. Spec `docs/plans/active/2026-09-19-pmoc-os-phase2.md`. |
| 2026-09-18 | **Executado (API, Slice 4):** shared `IWorkOrderGenerationService` for Hangfire + `POST /api/work-orders/from-plan`; Hangfire filter `IsActive && AutoGenerateEnabled`; `GET /api/work-orders?maintenancePlanId=`; `sourcePlanName` on `WorkOrderResponse`; 409 `DUPLICATE_WORK_ORDER`. Sem migration. Branch `feat/pmoc-os-phase1-generation`. Spec `docs/plans/active/2026-09-18-pmoc-os-phase1.md`. |
| 2026-09-18 | **Executado (API, Slice 3):** PMOC global template library on `feat/pmoc-os-phase1-template-library` — GET `/api/global-templates` Published-only (+ additive library fields); GET `/api/global-templates/{id}` including Deprecated; POST `/api/maintenance-plans/from-template` server clone (`OriginKind=RolvixTemplate`, frozen version, new PlanTask ids). Sem migration. Slices 1–2 on `develop`. Spec `docs/plans/active/2026-09-18-pmoc-os-phase1.md`. |
| 2026-09-18 | **Executado (API, Slice 2):** PMOC plan mutations — PUT `/api/maintenance-plans/{id}/tasks` replace-set; DELETE unused 204 / used 409 `PLAN_IN_USE`; lineage/auto on GET/create/update responses; header PUT `AutoGenerateEnabled`. Sem migration. Merged to `develop` (`5270bc1`, PR #81). Spec `docs/plans/active/2026-09-18-pmoc-os-phase1.md`. |
| 2026-09-18 | **Executado (API, Slice 1):** PMOC/OS Phase 1 foundation — template `LibraryKey`/`Version`/`Status`/`SourceReferences`; plan `OriginKind`/`AutoGenerateEnabled` (existing TRUE backfill, new FALSE default); WO `SourcePlanName` + FK Restrict; unique `ux_os_work_orders_pmoc_period`. Migrations `AddPmocOsPhase1Foundation` + `AddPmocOsWorkOrderPeriodUnique`. Hangfire filter unchanged. **Não aplicar em PROD da máquina.** Branch `feat/pmoc-os-phase1-foundation`. Spec `docs/plans/active/2026-09-18-pmoc-os-phase1.md`. |
