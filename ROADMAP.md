...
- [x] B2C self-cancel (código API): `POST /api/reservations/mine/{id}/cancel` (policy `Customer`, herda `[RequireActiveModule(Rentals)]`). Owner + `PendingDeposit`/`Confirmed` + `UtcNow < StartDateTime`. Occupancy release compartilhada com staff Cancel. Staff `POST /{id}/cancel` sem cutoff. Sem permission nova. Sem migration.
...
| Data | Mudança |
|---|---|
| 2026-09-07 | **Executado (API):** B2C self-cancel — `POST /api/reservations/mine/{id}/cancel` (`CancelByCustomerAsync`). Occupancy release compartilhada com staff Cancel. Cutoff só no Customer (`UtcNow >= StartDateTime` → 409). Staff cancel após início permanece. Sem migration. Branch `feat/rentals-b2c-self-cancel`. WEB ainda pendente. |
