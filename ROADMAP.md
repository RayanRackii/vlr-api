...
| Data | Mudança |
|---|---|
| 2026-09-18 | **Em progresso (API, Slice 2):** PMOC plan mutations on `feat/pmoc-os-phase1-plan-mutations` — PUT `/api/maintenance-plans/{id}/tasks` replace-set; DELETE unused 204 / used 409 `PLAN_IN_USE`; lineage/auto on GET/create/update responses; header PUT `AutoGenerateEnabled`. Sem migration. Slice 1 merged to `develop`. Spec `docs/plans/active/2026-09-18-pmoc-os-phase1.md`. |
